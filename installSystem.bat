...
:: CHANGE THESE 
set app_package=com.pukkol.apkcenter
set dir_app_name=Pukkol
set MAIN_ACTIVITY=ContentMain.MainActivity
...
:: Remove old version apk (time consuming)
rem %ADB_SH% ls %apk_target_sys%
rem if %ERRORLEVEL% == 0 (
rem 	%ADB_SH% rm %apk_target_sys% 
rem ) else (
rem 	echo ^^^ no old apk exists
rem )
...
:: reboot when older version heas been updated
rem if %RM_OLD% == 1 (
rem 	%ADB_SH% reboot
rem )
